...
const emailMap: { [key: string]: any } = {
  [UserAuthTypeEnum.register]: {
    subject: '注册 MagicTool 账号',
    html: (code: string) => `<div>您正在注册 MagicTool 账号，验证码为：${code}</div>`
  },
  [UserAuthTypeEnum.findPassword]: {
    subject: '修改 MagicTool 密码',
    html: (code: string) => `<div>您正在修改 MagicTool 账号密码，验证码为：${code}</div>`
  }
};
